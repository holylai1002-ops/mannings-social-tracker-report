...
var LI_COLORS = {
  ...
  desktop: '#5B73E8',
  mobile: '#43A047',
  palette: ['#FE8301', '#5B73E8', '#43A047', '#7CB5EC', '#674EAA'],
};

function liXAxisLabel() {
  return {
    show: true,
...
function renderLiDonut(containerId, data, labels, colors) {
  var chart = getChart(containerId);
  if (!chart || !data || data.length === 0) return;
  var total = data.reduce(function(s, d) { return s + (d.value || 0); }, 0);
  var mapped = labels.map(function(l, i) {
    var found = data.find(function(d) { return d.name.toLowerCase() === l.toLowerCase(); });
    return { name: l, value: found ? (found.value || 0) : 0, itemStyle: { color: colors[i] } };
  });
  chart.setOption({
    title: { text: total.toLocaleString(), left: '50%', top: 'center', textAlign: 'center', textVerticalAlign: 'middle', textStyle: { fontSize: 20, fontWeight: 'bold', color: '#18181B' } },
    tooltip: { trigger: 'item', formatter: '<b>{b}</b><br/>Count: {c} ({d}%)' },
    legend: { orient: 'horizontal', bottom: 5, left: 'center', textStyle: { fontSize: 11 }, itemWidth: 10, itemHeight: 10 },
    series: [{
      type: 'pie', radius: ['42%', '62%'], center: ['50%', '46%'], avoidLabelOverlap: true,
...
function renderLiNetFollowers(containerId, data) {
  var chart = getChart(containerId);
  if (!chart || !data || !data.dates) return;
  var xLabels = data.dates.map(function(d) { return fmtDate(d); });
  chart.setOption({
    tooltip: { trigger: 'axis', axisPointer: { type: 'shadow' } },
    legend: { bottom: 5, left: 'center', textStyle: { fontSize: 11 }, itemWidth: 10, itemHeight: 10 },
...
function renderLiLine(containerId, data, color, label) {
  var chart = getChart(containerId);
  if (!chart || !data || !data.dates) return;
  var xLabels = data.dates.map(function(d) { return fmtDate(d); });
  var vals = data.values || [];
  chart.setOption({
    tooltip: { trigger: 'axis', formatter: function(p) { return '<b>' + p[0].name + '</b><br/>' + (label || 'Value') + ': ' + p[0].value.toLocaleString(); } },
    grid: { left: 50, right: 20, top: 20, bottom: 60 },
    xAxis: { type: 'category', data: xLabels, axisLabel: liXAxisLabel(), axisLine: { lineStyle: { color: '#E4E4E7' } } },
    yAxis: { type: 'value', axisLabel: { fontSize: 10, color: '#6E6F75', formatter: function(v) { return v.toLocaleString(); } }, splitLine: { lineStyle: { color: '#F0F0F3' } } },
    series: [{
      type: 'line', data: vals, smooth: true, symbol: 'circle', symbolSize: 4,
      lineStyle: { color: color, width: 2 },
      itemStyle: { color: color },
      areaStyle: { color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [{ offset: 0, color: color + '33' }, { offset: 1, color: color + '03' }]) },
    }],
  });
}

function renderLiSocialActions(containerId, data) {
  var chart = getChart(containerId);
  if (!chart || !data || !data.dates) return;
  var xLabels = data.dates.map(function(d) { return fmtDate(d); });
  chart.setOption({
...
function renderLiHBar(containerId, data, color) {
  var chart = getChart(containerId);
  if (!chart || !data || data.length === 0) return;
  var sorted = data.slice().sort(function(a, b) { return (a.value || 0) - (b.value || 0); });
  chart.setOption({
    tooltip: { trigger: 'axis', axisPointer: { type: 'shadow' }, formatter: function(p) { return '<b>' + p[0].name + '</b><br/>Followers: ' + p[0].value.toLocaleString(); } },
    grid: { left: 10, right: 50, top: 10, bottom: 10, containLabel: true },
    xAxis: { type: 'value', axisLabel: { fontSize: 10, color: '#6E6F75' }, splitLine: { lineStyle: { color: '#F0F0F3' } } },
    yAxis: { type: 'category', data: sorted.map(function(d) { return d.name; }), axisLabel: { fontSize: 11, color: '#52525B' }, axisLine: { lineStyle: { color: '#E4E4E7' } } },
    series: [{
      type: 'bar', data: sorted.map(function(d) { return { value: d.value, itemStyle: { color: color } }; }),
      label: { show: true, position: 'right', formatter: function(p) { return p.value.toLocaleString(); }, fontSize: 10, fontWeight: 600, color: '#18181B' },
      barWidth: '60%',
    }],
  });
}

function renderLiVBarMulti(containerId, data) {
  var chart = getChart(containerId);
  if (!chart || !data || data.length === 0) return;
  ...
  chart.setOption({
    ...
    xAxis: { type: 'category', data: sorted.map(function(d) { return d.name; }), axisLabel: { fontSize: 10, color: '#6E6F75', rotate: sorted.length > 4 ? 20 : 0, interval: 0 }, axisLine: { lineStyle: { color: '#E4E4E7' } } },
    yAxis: { type: 'value', axisLabel: { fontSize: 10, color: '#6E6F75' }, splitLine: { lineStyle: { color: '#F0F0F3' } } },
    series: [{
      type: 'bar',
      data: sorted.map(function(d, i) { return { value: d.value, itemStyle: { color: LI_COLORS.palette[i % LI_COLORS.palette.length] } }; }),
      barWidth: '50%',
      label: { show: true, position: 'top', formatter: function(p) { return p.value.toLocaleString(); }, fontSize: 10, fontWeight: 600, color: '#18181B' },
    }],
  });
}

function renderLiVisitorMetrics(containerId, data) {
  var chart = getChart(containerId);
  if (!chart || !data || !data.dates) return;
  var xLabels = data.dates.map(function(d) { return fmtDate(d); });
  chart.setOption({
    ...
    yAxis: { type: 'value', axisLabel: { fontSize: 10, color: '#6E6F75' }, splitLine: { lineStyle: { color: '#F0F0F3' } } },
    series: [
      { name: 'Desktop', type: 'line', data: data.desktop || [], smooth: true, symbol: 'circle', symbolSize: 4, lineStyle: { color: LI_COLORS.desktop, width: 2 }, itemStyle: { color: LI_COLORS.desktop } },
      { name: 'Mobile', type: 'line', data: data.mobile || [], smooth: true, symbol: 'circle', symbolSize: 4, lineStyle: { color: LI_COLORS.mobile, width: 2 }, itemStyle: { color: LI_COLORS.mobile } },
    ],
  });
}
