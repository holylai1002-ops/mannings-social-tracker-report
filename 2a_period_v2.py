"""
2a_period_v2.py - Community Comment Extraction (Target Period, Excel Output)
Based on 2a_new_period.py with:
...
OUTPUT_XLSX = BASE_DIR / f"Mannings_Comments_RAW_{TARGET_YEAR}_{TARGET_MONTH:02d}.xlsx"
...
def extract_comments_for_period():
    logging.info(f"=== 2a_period_v2: Comment Extraction ({TARGET_YEAR}-{TARGET_MONTH:02d}) ===")
    ...
    logging.info(f"Step: Filtering to target period {TARGET_YEAR}-{TARGET_MONTH:02d} ...")
    final_df = filter_to_target_period(final_df, TARGET_YEAR, TARGET_MONTH)

    if final_df.empty:
        logging.warning(f"No comments found for {TARGET_YEAR}-{TARGET_MONTH:02d}.")
        return None
    ...
    logging.info(f"DONE! {len(final_df)} comments extracted for {TARGET_YEAR}-{TARGET_MONTH:02d}.")
    return final_df

if __name__ == "__main__":
    extract_comments_for_period()
